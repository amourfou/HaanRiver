...
const GameUI: React.FC<GameUIProps> = ({ gameState, onPause, onResume }) => {
  return (
    <div className="absolute top-0 left-0 right-0 z-[60]">
      {/* 전체 화면 너비의 상단 배경 */}
      <div className="w-full bg-black bg-opacity-70 backdrop-blur-sm p-4">
        {/* 상단 UI */}
        <div className="flex justify-between items-center">
          ...
          {/* 일시정지 버튼 */}
          <motion.button
            className="rounded-lg p-3 text-white hover:bg-white hover:bg-opacity-20 transition-colors"
      ...
      {gameState.selectedViruses.length > 0 && (
        <motion.div
          className="w-full bg-black bg-opacity-70 backdrop-blur-sm p-4"
      ...
      {gameState.viruses.length === 0 || (gameState.viruses.length > 0 && gameState.viruses[0].y < 0) && (
        <motion.div
          className="w-full bg-black bg-opacity-70 backdrop-blur-sm p-3"
          initial={{ y: 50, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 0.5 }}
        >
